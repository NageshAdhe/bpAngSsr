...
const generateContrasts = require(path.resolve(__dirname, ('../plugins/utils/generate-contrasts')));
...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--trove-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--trove-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--trove-${name}-rgb))`;
    };
...
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--trove-${e(key)}`, value], [`--trove-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`trove.customProps.background.${colorScheme}`);
            const foreground = theme(`trove.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--trove-${e(key)}`, value], [`--trove-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--trove-${e(key)}`, value], [`--trove-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
